...
class Bullet:
  def __init__(self, centerY, centerX, Speed, size):
    self.centerX = centerX  #- Bullet"s X position
    self.centerY = centerY  #- Bullet"s Y position
    self.Speed = Speed  #- Bullet"s walk speed
    self.size = size  #- Bullet"s size
    ...
    self.BulletRect2 = self.Surface2.get_rect(x=Playerspawnlocationx, y=Playerspawnlocationy)  #- Set the position of the player
  def draw(self, Window):
    """ Drawing the player into the window """
    Window.blit(self.Surface2, self.BulletRect2)  #- Blit (draw) the player on the screen
...
Player1 = Players(Playerspawnlocationx, Playerspawnlocationy, 7, 10,700)  #- Initialize Player1
Player2 = Players(Playerspawnlocationx, Playerspawnlocationy, 5, 20,1000)  #- Initialize Player2
Player3 = Players(Playerspawnlocationx, Playerspawnlocationy, 5, 20,1000)  #- Initialize Player2

Wall1 = Wall(400, 300, 500, 300)  #- Initialize Wall1

Bullet1 = Bullet(Zombiespawnlocationx, Zombiespawnlocationy, 30, 20)  #- Initialize Bullet1
...
while MainLoop:
  ...
  def Program(PlayerClasses1, Zombiesclasses1, SprintBarX, OneWayWall, WalkInWalls,Bulletsclasses1):
    global Stamina
    global WindowWidth
    global WindowHeight
    global window_x
    ...
    # If the player collides with the zombie or the bullet, reduce the player's health
    if Zombiesclasses1.ZombieRect.colliderect(PlayerClasses1.PlayerRect2):
        PlayerClasses1.Health = PlayerClasses1.Health - 10
        
    if Zombiesclasses1.ZombieRect.colliderect(Bulletsclasses1.BulletRect2):
        PlayerClasses1.Health = PlayerClasses1.Health - 10,
    ...
    if PlayerClasses1.PlayerRect2.colliderect(OneWayWall.WallRect2):
        PlayerClasses1.PlayerRect2.y = PlayerClasses1.PlayerRect2.y + PlayerClasses1.WalkSpeed  #- Prevent player from walking through wall
        
    if Zombiesclasses1.ZombieRect.colliderect(OneWayWall.WallRect2):
        Zombiesclasses1.ZombieRect.y = Zombiesclasses1.ZombieRect.y + Zombiesclasses1.WalkSpeed  #- Prevent zombie from walking through wall
    ...
    if WalkInWalls == True:  
      ...
      if PlayerClasses1.PlayerRect2.x >= WindowWidth - PlayerClasses1.size:
          ...
          pygame.display.set_mode ([WindowWidth, WindowHeight])    #- Activate the window
      ...
      Bulletsclasses1.draw(Window)  #- Draw Zombie
      ...
      #pygame.draw.rect (Window, [255,0,0], (0,WindowHeight-100,SprintBarX,10)) #- Draw the ability bar

      #pygame.draw.rect (Window, [0,255,0], (0,WindowHeight-50,PlayerClasses1.Health,10)) #- Draw the health bar
  ...
  Program(Player2, Zombie2, SprintBarX, Wall1, WalkInWalls,Bullet1)  #- Call the game logic
